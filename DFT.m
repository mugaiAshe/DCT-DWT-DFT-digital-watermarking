...
%-----------------------读入图像-------------------------------------%
markbefore=imread('watermark.png');
...
cover_image=imread('cover.jpeg');
cover_image=rgb2gray(cover_image);
subplot(2,3,2),imshow(cover_image,[]),title('载体图像'); %[]表示显示时灰度范围为image上的灰度最小值到最大值
[crm,ccm]=size(cover_image);
before=blkproc(cover_image,[crm,ccm],'fft2');   %将载体图像的灰度层整体做二维FFT变换，结果记入矩阵before

imwrite(real(before),'before.bmp','bmp');
imwrite(cover_image,'cover.bmp','bmp');

I=mark;
alpha=100000;     %尺度因子,控制水印添加的强度,决定了频域系数被修改的幅度
...
imwrite(real(after),'after.bmp','bmp');
result=blkproc(after,[crm,ccm],'ifft2');    %将经处理的图像整体做二维FFT逆变换
result = real(result);
result = uint8(result);
imwrite(result,'傅里叶.jpeg','jpeg');      %存储添加水印后的图像
...
switch choice        %读入输入的选择  withmark为等待提取水印的图像
    case 1
        ...
        imwrite(withmark,'傅-噪声.jpeg','jpeg'); 
    case 2
         result_2=result;
    ...
    case 3
        ...
        title('上方剪切后图像');
        withmark=result_3;
        imwrite(withmark,'傅-剪切.jpeg','jpeg'); 
    case 4
        ...
        title('旋转10度后图像');
        withmark=result_4;
        imwrite(withmark,'傅-旋转.jpeg','jpeg'); 
    case 5
        result_5 = result; 
        ...
        title('经JPEG压缩后图像'); 
        withmark=WImage5cl;
        imwrite(withmark,'傅-缩放.jpeg','jpeg'); 
    case 6
        result_6=result;
...
after_2=blkproc(result,[crm,ccm],'fft2');   %此步开始提取水印，将灰度层进行DFT变换
after_2=real(after_2);
...
after_2=blkproc(withmark,[crm,ccm],'fft2');   %此步开始提取水印，将灰度层进行DFT变换
after_2=real(after_2);
p=zeros(1,8);        %初始化提取数值用的矩阵
mark_2 = zeros(rm,cm);
for i=1:rm
...
subplot(2,3,6);
imshow(mark),title('原提取水印');
imwrite(mark_2,'傅里叶水印.png','png'); 

error=mark_2-mark;
mse=mse(error);
psnr = 10 * log10(1/mse);
disp('原水印图像与提取水印图像的PSNR:')
disp(psnr);
NC=nc(mark_2,mark);  
