...
[before, cH, cV, cD]=dwt2(cover_image,'db1');   %将载体图像的灰度层做二维DWT变换，结果记入矩阵before
...
result=idwt2(after, cH, cV, cD,'db1');    %将经处理的图像做二维DWT逆变换
result = uint8(result);
imwrite(result,'离散小波.jpeg','jpeg');      %存储添加水印后的图像
...
switch choice        %读入输入的选择  withmark为等待提取水印的图像
    case 1
        ...
        imwrite(withmark,'波-噪声.jpeg','jpeg'); 
    case 2
         result_2=result;
    ...
    case 3
        ...
        title('上方剪切后图像');
        withmark=result_3;
        imwrite(withmark,'波-剪切.jpeg','jpeg'); 
    case 4
        ...
        title('旋转10度后图像');
        withmark=result_4;
        imwrite(withmark,'波-旋转.jpeg','jpeg'); 
    case 5
        result_5 = result; 
        ...
        title('经JPEG压缩后图像'); 
        withmark=WImage5cl;
        imwrite(withmark,'波-缩放.jpeg','jpeg'); 
    case 6
        result_6=result;
...
imwrite(withmark,'傅-攻击.jpeg','jpeg'); 
%------------------------水印提取-----------------------------%
%

[after_2, cH, cV, cD]=dwt2(result,'db1');   %此步开始提取水印，将灰度层进行DWT变换
...
[after_2, cH, cV, cD]=dwt2(withmark,'db1');   %此步开始提取水印，将灰度层进行DWT变换
after_2=real(after_2);
p=zeros(1,8);        %初始化提取数值用的矩阵
mark_2 = zeros(rm,cm);
for i=1:rm
